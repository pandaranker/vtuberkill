    ...
    return {
        ...
        skill: {
            ...
            zhimao: {
                ...
                filter(event, player) {
                    if (!event.player || event.player == player)
                        return false;
                    if (get.distance(event.player, player, 'pure') == 1)
                        return false;
                ...
                content() {
                    ...
                    if (target.getEquip(1)) {
                        player.chooseControlList(['取消之并抽一张牌',
                            '获得' + get.translation(target) + '的武器牌，视为对其使用【杀】'], true, function (event, player) {
            ...
            gutai: {
                trigger: { global: 'damageEnd' },
                filter(event, player) {
                    if (event.player != player && event.getParent().player != player)
                        return false;
                    ...
                    dam: {
                        ...
                        filter(event, player) {
                            ...
                            player.storage.caibu.forEach(function (c) {
                                if (get.suit(c) == get.suit(event.card))
                                    chk = true;
                            });
                            return chk;
                        },
            ...
            jiumao: {
                ...
                content() {
                    ...
                    event.target.chooseCard('he', [1, Infinity]).set('ai', card => {
                        var player = _status.event.player;
                ...
                subSkill: {
                    ...
                    lose: {
                        ...
                        filter(event, player) {
                            ...
                            if (!maoliang.length)
                                return false;
                            return event.ss.filter(card => maoliang.contains(card)).length > 0;
                        },
                        content() {
            ...
            enfan: {
                ...
                content() {
                    'step 0';
                    event.target = trigger.player;
                    player.chooseCard('s', [1, Infinity], card => card.hasGaintag('maoliang')).set('logSkill', ['enfan', event.target]).set('ai', card => {
                        var target = _status.event.target;
            ...
            shiqi: {
                ...
                zhuSkill: true,
                filter(event, player) {
                    if (!player.hasZhuSkill('shiqi') || event.player.group != player.group)
                        return false;
                    return true;
                },
                logTarget: 'player',
            ...
            dunzou: {
                ...
                content() {
                    player.addTempSkill('dunzou_enable', { target: 'phaseBegin' });
                    game.broadcastAll(function (splayer) {
            ...
            dianyinchuancheng: {
                ...
                content() {
                    ...
                    if (event.target) {
                        ...
                        if (max > event.target.countCards('h'))
                            event.target.gain(get.cards(max - event.target.countCards('h')), 'draw', 'log');
                        if (event.count)
                            event.goto(1);
                    }
                },
            ...
            zengzhi: {
                ...
                priority: 41,
                filter(event, player) {
                    if (!player.isPhaseUsing() || !event.card.isCard)
                        return false;
                    var card = event.card;
                    ...
                    if (event.targets && !info.multitarget) {
                        if (game.hasPlayer(function (cur) {
                            return event.targets.contains(cur) && lib.filter.targetEnabled2(card, player, cur);
                        })) {
                            return true;
                        }
                    }
            ...
            zhengtibuming: {
                ...
                content() {
                    ...
                    event.configPrompt = (id, prompt) => {
                        if (!event.dialog || event.dialog.content.childNodes[0].innerHTML == prompt)
                            return;
                        if (player.isOnline2()) {
                    ...
                    if (result.bool) {
                        ...
                        list.push(opts[2]);
                        if (!list.contains(event.aiChoiceSkill))
                            event.aiOpt = list[0];
                        else
                            event.aiOpt = event.aiChoiceSkill;
            ...
            mingyunniezao: {
                ...
                popup: false,
                filter(event, player) {
                    return player.hasZhuSkill('mingyunniezao') && event.player.group == player.group && event.player != player;
                },
                content() {
                    "step 0";
                    ...
                    if (result.bool) {
                        ...
                        for (var i = 0; i < event.cards.length; ++i) {
                            if (event.cards[i] == event.replaceCard)
                                continue;
                    ...
                    if (result.bool) {
                        ...
                        player.respond(event.replaceCard, 'mingyunniezao', 'highlight');
                        ...
                        trigger.orderingCards.addArray([event.replaceCard]);
                ...
                subSkill: {
                    gainHand: {
                        ...
                        content() {
                            ...
                            if (cnt > 0) {
                                player.chooseCard('he', cnt, '交给' + get.translation(event.p1) + get.cnNumber(cnt) + '张牌', true).set('ai', card => {
            ...
            mozouqiyin: {
                ...
                direct: true,
                init(player) {
                    if (!player.storage.mozouqiyin)
                        player.storage.mozouqiyin = '小';
                },
                filter(event, player) {
                ...
                content() {
                    ...
                    for (var i = 0; i < history.length; ++i) {
                        if (event.sourceDamageHistory.contains(history[i]))
                            continue;
            ...
            budingpaidui: {
                ...
                mark: true,
                init(player) {
                    if (!player.storage.budingpaidui)
                        player.storage.budingpaidui = {};
                    if (!player.storage.budingpaidui.current)
                        player.storage.budingpaidui.current = '小';
                    if (!player.storage.budingpaidui.left)
                ...
                checkNumber(lstNum, curNum, item) {
                    ...
                    if (typeof item == 'string')
                        ...
                        if (!player || !player.storage.budingpaidui || !player.storage.budingpaidui.current)
                            return false;
                ...
                content() {
                    ...
                    player.markSkill('budingpaidui');
                    if (player.storage.mozouqiyin)
                        player.storage.mozouqiyin = result.control;
            ...
            _usedCardRecord: {
                ...
                log: false,
                content() {
                    if (!player.storage._usedCardRecord)
                        player.storage._usedCardRecord = [];
        ...
        translate: {
            ...
            kaguraNaNa: `神乐七奈`,
            DDzhanshou: `DD斩首`,
            DDzhanshou_info: `当你使用牌指定目标后，你可选择其中一名目标角色，该角色每满足一项你便可将其一张牌移出游戏直到此回合结束：
			手牌数不少于你；体力值不少于你；装备区牌数不少于你。然后若该角色没有手牌，其摸一张牌。`,
