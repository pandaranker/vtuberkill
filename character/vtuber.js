    ...
    return {
        ...
        skill: {
            ...
            qixu2: {
                ...
                content() {
                    ...
                    if (event.current == player) {
                        ...
                        var next = event.current.chooseCard(get.translation(player) + '对' + get.translation(target) + '使用一张杀，是否替弃置一张杀阻止', function (card, player, event) {
                            event = event || _status.event;
                            return card.name == 'sha';
                        }, { name: 'sha' }, 1);
                        next.set('ai', card => {
            ...
            rongyaochengyuan: {
                ...
                filter(event, player) {
                    if (event.source == undefined || event.source == player)
                        return false;
                    if (event.source.hasSkill('rongyaochengyuan_homolive'))
                        return false;
                    return true;
                },
            ...
            zhongxinghezou: {
                ...
                content() {
                    ...
                    if (result.bool && result.targets[0]) {
                        ...
                        event.starget.chooseCard(true, 'h', '众星合奏：亮出一张手牌').set('ai', card => {
                ...
                content() {
                    ...
                    if (result.bool) {
                        ...
                        if (event.num > 0) {
                            ...
                            if (player.hasSkillTag('viewHandcard', null, event.target, true))
                                rand = 1;
                            var list = ['0张'];
                        ...
                        mod: {
                            playerEnabled(card, player, target) {
                                ...
                                for (var i of players) {
                                    if (target == player && i.storage.yueyao == player.countCards('h'))
                                        return false;
                                }
                            },
                        }
            ...
            jiajiupaidui: {
                ...
                enable: 'chooseToUse',
                filter(event, player) {
                    if (player.hasSkill('jiajiupaidui_tag'))
                        return false;
            ...
            liuhua: {
                ...
                subSkill: {
                    regain: {
                        ...
                        direct: true,
                        filter(event, player) {
                            if ((event.name == 'phase' && event.skill != 'liuhua') || (event.name == 'turnOver' && (!event.getParent()._trigger || event.getParent()._trigger.skill != 'liuhua')))
                                return false;
                        ...
                        content() {
                            ...
                            if (result.bool) {
                                ...
                                event.cards = player.storage.liuhua.filter(card => get.color(card) == get.color(result.links[0]));
