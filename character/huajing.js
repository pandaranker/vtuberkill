        ...
        skill: {
            ...
            xieqi: {
                ...
                check(card) {
                    ...
                    if (!evt.xieqi_choice)
                        evt.xieqi_choice = lib.skill.xieqi.getResult(evt.player.getCards('h'));
            ...
            pobing: {
                ...
                content() {
                    ...
                    if (event.target.hp > player.hp)
                        num++;
                    if (event.target.getHistory('changeHujia').length)
                        num++;
            ...
            sp_guiliu: {
                    ...
                    for (var i = 0; i < event.cards.length; i++) {
                        if (get.position(event.cards[i], true) == 'd') {
                            for (var j = 0; j < cards.length; j++) {
                                if (get.color(event.cards[i]) == get.color(cards[j]))
                                    return true;
                            }
                        }
                    }
                ...
                content() {
                    ...
                    if (!event.cards2[event.numx])
                        event.cards2[event.numx] = [];
                    event.cards2[event.numx].push(card);
                    while (event.numx--) {
                        var card = game.createCard(trigger.cards[event.num].name, trigger.cards[event.num].suit, trigger.cards[event.num].number, trigger.cards[event.num].nature);
                        for (var i = 0; i < trigger.cards[event.num]['classList'].length; i++) {
                            if (!card.classList.contains(trigger.cards[event.num]['classList'][i]))
                                card.classList.add(trigger.cards[event.num]['classList'][i]);
                        }
