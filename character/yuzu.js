        ...
        skill: {
            ...
            xiangnuo2: {
                ...
                check(card) {
                    ...
                    if (!evt.minsi_choice.contains(card))
                        return 0;
                    return 1;
                },
                content() {
                ...
                content() {
                    ...
                    chooseButton.set('ai', function (button) {
                        ...
                        for (let i of ui.selected.buttons) {
                            let card = i.link;
                            if (target.getCards('he').contains(card))
                                ts.push(card);
                            else
                                ps.push(card);
                        }
            ...
            shuipo: {
                ...
                filter(event, player) {
                    if (event.name == 'useCard')
                        return get.type2(event.card) == 'trick' && !player.hasSkill('shuipo_used');
                    ...
                    if (event.name == 'loseHp')
                        ...
                        return player.getHistory('lose', evt => {
                            ...
                            for (let i in evt.gaintag_map) {
                                if (evt.gaintag_map[i].contains('ming_'))
                                    return true;
                            }
                            return false;
                        }).length > 0;
                    ...
                    if (trigger.name == 'loseHp') {
                        ...
                        event.cards = trigger.cards.filter(card => player.getHistory('lose', evt => {
                            if (evt.getParent() != trigger)
                                return false;
                            if (evt.gaintag_map[card.cardid] && evt.gaintag_map[card.cardid].contains('ming_'))
                                return true;
                            return false;
                        }).length > 0);
            ...
            yankui: {
                ...
                filter(event, player) {
                    return player != event.player && player.countCards('he', card => !player.storage.yankui_mark
                        || !player.storage.yankui_mark.contains(get.type2(card))) > 1 && event.player.countGainableCards(player, 'h');
                },
                check(event, player) {
                ...
                content() {
                    ...
                    game.broadcastAll(function (player) {
                        player.chooseToDiscard(get.prompt2('yankui'), 'he', card => !player.storage.yankui_mark
                            || !player.storage.yankui_mark.contains(get.type2(card))).ai = card => {
            ...
            tiantang: {
                ...
                filter(event, player) {
                    if (player.countCards('he') < (event.player.storage.paryi || 1))
                        return false;
                    return true;
                },
                check(event, player) {
                    if (player.storage.haoren !== true)
                        return (event.player.storage.paryi || 1) <= 2 && get.attitude(player, event.player) < 1 && !event.player.hasJudge('lebu');
                ...
                content() {
                    ...
                    if (result.bool) {
                        ...
                        let list = ['观看' + get.translation(event.target) + '的手牌' + (player.awakenedSkills.contains('haoren') ? '并重铸' : '并弃置') + '其一张' + get.translation(event.tiantang) + '牌，令其执行一个额外的出牌阶段',
                            '令' + get.translation(event.target) + '摸两张牌，然后其只能使用' + get.translation(event.tiantang) + '的牌直到回合结束'];
                        // if(!player.awakenedSkills.contains('haoren')&&!event.target.countDiscardableCards(player,'he'))	list.shift();
            ...
            huange: {
                ...
                check(event, player) {
                    if (event.player != player && get.attitude(player, event.player) < 0 && event.player.inRange(player))
                        return true;
                ...
                subSkill: {
                    date: {
                        ...
                        filter(event, player) {
                            if (player.storage.qishi === true)
                                return false;
                            if (player.storage.qishi_date && player.storage.qishi_date.contains(event.player))
                                return false;
                            return true;
                        },
                        content() {
                    ...
                    drawBy: {
                        ...
                        filter(event, player) {
                            ...
                            if (get.type2(event.card) != 'trick')
                                return false;
                            if ((_b = (_a = player.storage) === null || _a === void 0 ? void 0 : _a.tianyi_drawBy) === null || _b === void 0 ? void 0 : _b.contains(get.suit(event.card)))
                                return false;
            ...
            yeyu: {
                ...
                subSkill: {
                    sha: {
                        ...
                        popup: false,
                        filter(event, player) {
                            if (event.player == player || get.name(event.card) != 'sha')
                                return false;
                    ...
                    trick: {
                        ...
                        filter(event, player) {
                            ...
                            if (event.player == player || get.type(event.card) != 'trick')
                                return false;
                            return ((_a = event === null || event === void 0 ? void 0 : event.targets) === null || _a === void 0 ? void 0 : _a.length) && player.countCards('h', { suit: 'club' });
                        },
            ...
            huawen: {
                ...
                content() {
                    ...
                    next.set('switchToAuto', function () {
                        ...
                        let links = [];
                        if (get.attitude(player, target) < 0) {
                            let saves = list2.filter(card => ['tao', 'jiu', 'zong'].contains(get.name(card)));
                    ...
                    next.set('process', function (selected, now) {
                        ...
                        let overOne = 0;
                        if (last.length % 2 == 1) {
                            let pack = selected[selected.length - 1];
                        }
                        ...
                        if (!pack) {
                            if (list1.contains(now)) {
                                for (let i = 0; i < list2.length; i++) {
                                    for (let j in over) {
                                        if (over[j] + overOne >= 1 || (going.contains(j) && going.length * 2 <= last.length))
                                            continue;
                    ...
                    if (result.bool && result.links) {
                        ...
                        cards1 = cards1.filter(card => event.list1.contains(card));
                        cards2 = cards2.filter(card => event.list2.contains(card));
            ...
            liaohu: {
                ...
                check(event, player) {
                    var _a, _b;
                    return get.recoverEffect(((_b = (_a = player.storage) === null || _a === void 0 ? void 0 : _a.huawen) === null || _b === void 0 ? void 0 : _b.length) ? player.storage.huawen[0] : player, player, player);
                },
                content() {
                    var _a, _b;
            ...
            huangran: {
                ...
                subSkill: {
                    drawBy: {
                        ...
                        forced: true,
                        filter(event, player) {
                            if (event.player.hasSkill('huangran_shao'))
                                return false;
            ...
            yinzhen: {
                ...
                subSkill: {
                    ...
                    getC: {
                        ...
                        popup: false,
                        content() {
                            for (let i = 0; i < (player.storage.yinzhen_contains.length); i += 2) {
            ...
            tiaolian: {
                ...
                filter(event, player) {
                    ...
                    if (event.player == player && !event.targets.filter(cur => {
                        ...
                        return false;
                    if (event.player != player && !player.canCompare(event.player))
                        return false;
                    return player.countCards('h') > 0;
                },
                check(event, player) {
                    ...
                    drawBy: {
                        ...
                        filter(event, player) {
                            ...
                            if (evt.name == '_lianhuan')
                                evt = event.getTrigger().getParent(2);
                            else
                                evt = evt.getParent();
                            if (evt.addedSkill && evt.addedSkill.contains('liebo'))
                                return false;
                            return get.color(event.card, player) == 'black';
                        },
                        priority: 12,
                    ...
                    lose: {
                        ...
                        filter(event, player) {
                            for (let i in event.gaintag_map) {
                                if (event.gaintag_map[i].contains('zhongjizhimeng'))
                                    return true;
                            }
                        },
                        direct: true,
            ...
            chengneng: {
                ...
                check(event, player) {
                    if (event.num == 1)
                        return get.recoverEffect(event.player, player, player);
                    return 0;
                },
                content() {
                    'step 0';
            ...
            mudu: {
                ...
                filter(event, player) {
                    if (event.player == player || event.player.countCards('he') < 2)
                        return false;
            ...
            yingkuo: {
                ...
                direct: true,
                filter(event, player) {
                    if (event.name == 'equip' && event.swapped)
                        return false;
                    ...
                    cardsDis: {
                        ...
                        filter(event, player) {
                            ...
                            return event.cards.filter(card => get.position(card, true) == 'd'
                                && ['basic', 'trick'].contains(get.type(card))).length > 0;
                        },
                        content() {
                            let cards = trigger.cards.filter(card => get.position(card, true) == 'd' && ['basic', 'trick'].contains(get.type(card)));
            ...
            zhuqiao: {
                ...
                check(card, cards) {
                    let player = _status.event.player;
                    if (player.storage.zhuqiao_addCard && player.storage.zhuqiao_addCard.contains(get.suit(card)))
                        return 6 - get.value(card);
            ...
            tuncai: {
                ...
                filter(event, player) {
                    if (event.name == 'draw')
                        return event.player != player && player.storage.tuncai;
                    else
                        return event.cards.length && !player.storage.tuncai;
                },
                check(event, player) {
            ...
            zhidu: {
                ...
                zhuSkill: true,
                filter(event, player) {
                    if (!player.hasZhuSkill('zhidu') || event.player.group != player.group)
                        return false;
                    ...
                    cardDisable: {
                        ...
                        mod: {
                            cardEnabled2(card, player) {
                                if (player.storage.celv_cardDisable.contains(get.name(card, player)))
                                    return false;
                            },
                        },
                    }
            ...
            mianmo: {
                ...
                content() {
                    ...
                    if (result.bool) {
                        ...
                        if (player.storage.tiaolv_up && player.storage.tiaolv_up.contains(event.card))
                            event.goto(4);
                        if (player.storage.tiaolv_down && player.storage.tiaolv_down.contains(event.card))
                            event.goto(6);
                    }
                    else {
                        event.finish();
                    ...
                    player.chooseBool('眠魔：是否令目标各摸一张牌？').set('ai', function () {
                        let player = _status.event.player;
                        if (_status.event.targets.contains(player))
                            return true;
                        return false;
                ...
                subSkill: {
                    up: {
                        ...
                        content() {
                            if (player.storage[event.name].length)
                                player.storage[event.name].length = 0;
                        },
                        mod: {
                            number(card, player, number) {
                                let num = player.getDamagedHp() || 1;
                                if (player.storage.tiaolv_up.contains(card))
                                    return number + num;
                            },
                        },
                    },
                    down: {
                        ...
                        mod: {
                            number(card, player, number) {
                                let num = player.getDamagedHp() || 1;
                                if (player.storage.tiaolv_down.contains(card))
                                    return number - num;
                            },
                        },
                    },
            ...
            ming_niwei: {
                ...
                priority: 3,
                filter(event, player) {
                    if (event.player.hasSkill('niwei') && event.player.storage.niwei && event.player.storage.niwei.contains(event.name))
                        return true;
                    if (event.player != player)
                        return false;
                    ...
                    if (loser.getParent()) {
                        if (event.getParent() == loser.getParent()) {
                            for (let i in loser.gaintag_map) {
                                if (loser.gaintag_map[i].contains('ming_niwei'))
                                    return true;
                            }
                        }
                    }
            ...
            weizeng: {
                ...
                content() {
                    ...
                    player.chooseCard(get.prompt2('weizeng'), [1, Infinity], card => {
                        ...
                        if (card.hasGaintag('ming_niwei') || ['shan', 'tao'].contains(get.name(card)))
                            return 0;
                        return random() - 0.1;
                    });
                    'step 1';
            ...
            aswusheng: {
                ...
                logTarget(event, player) {
                    if (event.name == 'respond')
                        return event.source;
                    if (['sha', 'qi', 'jiu', 'tao'].contains(event.card.name))
            ...
            quanyu: {
                ...
                check(event, player) {
                    ...
                    if (player.hp < num)
                        return false;
                    if (['shandian', 'du'].contains(event.card.name))
                        return false;
                    if (event.targets && event.targets.length && get.attitude(player, event.player) < 0) {
                        for (let i = 0; i < event.targets.length; i++) {
                            if (get.effect(event.targets[i], event.card, event.player, player) < 0)
                                return true;
                        }
                    }
                    else {
            ...
            fengqing: {
                ...
                content() {
                    ...
                    if ((_a = result.targets) === null || _a === void 0 ? void 0 : _a.length) {
                        ...
                        switch (event.change) {
                            case 1:
                                event.target.addSkill('fengqing_jiu');
                                break;
                            case 2:
                                event.target.addSkill('fengqing_tao');
                                break;
                            case 3: {
                                event.target.skip('phaseJudge');
                                event.target.skip('phaseDiscard');
                ...
                subSkill: {
                    mark: {
                        ...
                        filter(event, player) {
                            ...
                            if (event.name == 'cardsDiscard' && (event.getParent().name != 'orderingDiscard'
                                || (!((_a = event.getParent().relatedEvent) === null || _a === void 0 ? void 0 : _a.player) || event.getParent().relatedEvent.name == 'judge'
                                    || !check(event.getParent().relatedEvent.player))))
                                return false;
                            if (event.name == 'lose' && (event.position != ui.discardPile
                                || !check(event.player)))
                                return false;
                            let list = event.cards.filter(card => {
                                if (event.js && event.js.contains(card))
                                    return false;
                                for (let i = 0; i < record.length; i++) {
                                    if (player.storage.shixi_mark && player.storage.shixi_mark.contains(record[i]))
                                        continue;
                        ...
                        content() {
                            ...
                            let list = trigger.cards.filter(card => {
                                for (let i of record) {
                                    if (player.storage.shixi_mark && player.storage.shixi_mark.contains(i))
                                        continue;
                            ...
                            if (event.list[event.num]) {
                                if (player.storage.shixi_mark)
                                    event.record.removeArray(player.storage.shixi_mark);
            ...
            zhiyue: {
                ...
                filter(event, player) {
                    if (player.storage.juehuo.ans && player.storage.juehuo.ms) {
                        let card = event.card, list1 = player.storage.juehuo.ans.slice(0), list2 = player.storage.juehuo.ms.slice(0);
                        for (let i = 0; i < list1.length; i++) {
                        ...
                        if (evt.list1 && evt.list1.length && evt.list1.contains(now)) {
                            ...
                            if (selected.length) {
                                for (let i = 0; i < selected.length; i++) {
                                    if (evt.list1.contains(selected[i].link))
                                        return false;
                                }
                            }
            ...
            zhengniu: {
                ...
                filter(event, player) {
                    if (!player.storage.juehuo
                        ...
                        return false;
                    if (!event.source || get.itemtype(event.source) != 'player' || event.source == player)
                        return false;
                    if (event.name == 'link')
                ...
                content() {
                    'step 0';
                    event.target = trigger.source;
                    let check = get.attitude(player, event.target) > 0, list1 = player.storage.juehuo.ans.slice(0), list2 = player.storage.juehuo.ms.slice(0);
                    let list = ['###' + get.prompt('zhengniu') + '###选择交给' + get.translation(event.target) + '的绝活'];
                    if (list1.length) {
                        list.push('暗置绝活');
                        list.push([list1, 'card']);
                    }
            ...
            shshenyan: {
                ...
                content() {
                    ...
                    player.chooseCard('h', '『神言』:弃置一种牌名的牌', true).set('ai', card => {
                        if (['sha'].contains(card.name))
                            return 5;
                        if (!['sha', 'tao'].contains(card.name) && get.type(card) == 'basic')
                            return 6 - get.value(card);
                        return 1;
                    });
                    'step 2';
                    ...
                    shaMiss: {
                        ...
                        content() {
                            ...
                            player.chooseTarget('『吟虚』：令你或' + get.translation(trigger.target) + '调整手牌至上限', function (card, player, target) {
                                if (![player, _status.event.target0].contains(target))
                                    return false;
                ...
                ai: {
                    ...
                    result: {
                        target(player, target) {
                            ...
                            if (target.hp == 1 && suits.contains('heart'))
                                return -1;
                            let result = 0;
            ...
            baifei: {
                ...
                filter(event, player) {
                    ...
                    if (player == event.source)
                        return !characters.contains(event.player);
                    else if (event.source)
                        return !characters.contains(event.source);
                },
                content() {
                    'step 0';
            ...
            zhengen: {
                ...
                filter(event, player) {
                    ...
                    if (player == event.source)
                        return player.countCards('h') && event.player.countDiscardableCards(player, 'ej') && !characters.contains(event.player);
                    else if (event.source)
                        return player.countCards('h') && event.source.countDiscardableCards(player, 'ej') && !characters.contains(event.source);
                },
                content() {
                    'step 0';
                ...
                content() {
                    ...
                    if (_status.characterlist) {
                        ...
                        for (let i of _status.characterlist) {
                            ...
                            if (info[1] == 'psp' || info[4].contains('P_SP'))
                                list.push(i);
                        }
                    }
                        ...
                        list = get.charactersOL(function (i) {
                            let info = lib.character[i];
                            return !(info[1] == 'psp' || info[4].contains('P_SP'));
                        });
                    }
                    else {
                ...
                subSkill: {
                    going: {
                        ...
                        viewAsFilter(player) {
                            ...
                            for (let i of cards) {
                                ...
                                if (filter({ name: name, suit: suit }, player, event))
                                    return true;
                            }
                            return false;
                        },
                    ...
                    change: {
                        ...
                        filter(event, player) {
                            ...
                            if (name == 'useSkill')
                                name = event.skill;
                            if (['erni_change', 'erni_going'].contains(name))
                                return false;
            ...
            shouru: {
                ...
                filter(event, player) {
                    if (player.hasSkill('shouru_used'))
                        return false;
                    return (event.name == 'damage' || ['useCard', 'respond'].contains(event.name) && event.skill == 'erni_going') && game.hasPlayer(cur => {
            ...
            luqiu: {
                ...
                filter(event, player) {
                    ...
                    if (!record)
                        return false;
                    return event.cards && event.cards.filter(card => get.position(card, true) == 'd' && record.contains(get.suit(card))).length;
                },
                direct: true,
                content() {
                    'step 0';
            ...
            yangyao: {
                ...
                filter(event, player) {
                    ...
                    for (let i = 0; i < ui.discardPile.childElementCount; i++) {
                        ...
                        if (player.getStorage('yangyao').contains(get.name(card)))
                            continue;
                        if (get.type2(card) == 'trick') {
                            list.push(card);
                        }
            ...
            akjianwu: {
                ...
                content() {
                    ...
                    if (result.winner && result.loser) {
                        ...
                        if (event.card.cards && get.value(event.card.cards, event.winner, 'raw') > event.loser.countGainableCards(event.winner, 'he'))
                            check = 0;
                    ...
                    if (result.links) {
                        event.winner.chooseToUse({
                            cards: result.links,
                            filterCard(card) {
                                if (get.itemtype(card) != 'card' || !_status.event.cards || !_status.event.cards.contains(card))
                                    return false;
                ...
                content() {
                    ...
                    next.set('filterButton', function (button) {
                        ...
                        if (ul > 0) {
                            ...
                            let now = button.link;
                            if (evt.list2.contains(pre) && evt.list1.contains(now)) {
                                return true;
                            }
                            if (evt.list1.contains(pre) && evt.list2.contains(now)) {
                                return true;
                            }
                            return false;
                        }
                    ...
                    'step 1';
                    if (result.bool) {
                        event.cards1 = event.list1.filter(card => result.links.contains(card));
                        event.cards2 = event.list2.filter(card => result.links.contains(card));
                        ...
                        event.finish();
                    'step 2';
                    let cards = event.cards.map(card => event.cards1.contains(card) ? event.cards2[event.cards1.indexOf(card)] : card);
                    while (cards.length) {
            ...
            qianjiwanbian: {
                ...
                filter(event, player) {
                    if (event.name == 'damage' && event.getParent() && event.getParent().name != "trigger" && event.getParent(2) && event.getParent(2).qianjiwanbian)
                        return false;
                    return true;
                },
                    ...
                    if (result.control != '取消') {
                        ...
                        let list = get.gainableSkills(function (info, skill) {
                            ...
                            for (let i = 0; i < name.length; i++) {
                                if (lib.skill.qianjiwanbian.gainable.contains(name.substring(i, i + 1)))
                                    return true;
                            }
                        });
                    ...
                    mark: {
                        ...
                        filter(event, player) {
                            ...
                            player.getHistory('lose', evt => {
                                if ((evt === null || evt === void 0 ? void 0 : evt.type) == 'discard' && evt.getParent('phaseDiscard') == event && evt.hs)
                                    cards.addArray(evt.cards);
                            });
            ...
            liying: {
                ...
                filter(event, player) {
                    ...
                    if (event.name == 'damage')
                        source = event.source;
                    else if (Array.isArray(event.respondTo))
                        source = event.respondTo[0];
                    if (source) {
                    ...
                    if ((_a = result.cards) === null || _a === void 0 ? void 0 : _a.length) {
                        ...
                        switch (color) {
                            case 'red':
                                ...
                                let info = get.info(card);
                                if (['basic', 'trick'].contains(info.type) && info.allowMultiple !== false
                                    && trigger.targets && !info.multitarget) {
                                    if (game.hasPlayer(cur => !trigger.targets.contains(cur) && lib.filter.targetEnabled2(card, player, cur))) {
            ...
            butaizhun: {
                ...
                filter(event, player) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        if (event.filterCard({ name: list[i] }, player))
                            return true;
                    }
                    return false;
                },
            ...
            butaizhun_respond: {
                ...
                filter(event, player) {
                    ...
                    if (event.responded)
                        return false;
                    if (!event.filterCard({ name: 'shan' }) && !event.filterCard({ name: 'sha' }))
                        return false;
                    if (!lib.filter.cardRespondable({ name: 'shan' }, player, event) && !lib.filter.cardRespondable({ name: 'sha' }, player, event))
                        return false;
                    if (!player.countCards('h'))
                        return false;
                    return true;
            ...
            rouqing: {
                ...
                content() {
					...
					然后获得未被选择的牌`).set('ai', function (button) {
                        let min = 0;
                        if (ui.selected.buttons.length >= event.min)
                            min = -5;
            ...
            guangying: {
                ...
                subSkill: {
                    recoverBy: {
                        ...
                        filter(event, player) {
                            var _a;
                            return ((_a = event.cards) === null || _a === void 0 ? void 0 : _a.length) >= 4;
                        },
                        forced: true,
                        content() {
                            player.recover();
            ...
            mengnan: {
                ...
                filter(event, player) {
                    var _a;
                    if (event.name == 'addJudge' && event.player == player)
                        return true;
                    let evt = event.getl(player);
                    ...
                    shiyue: {
                        ...
                        filter(event, player) {
                            if (!player.storage.jusheng_shiyue.isIn())
                                return false;
                            return event.targets.contains(player) || event.targets.contains(player.storage.jusheng_shiyue);
                        },
                        direct: true,
                        lastDo: true,
                        content() {
                            ...
                            effect: {
                                player(card, player, target, current) {
                                    if (player.storage.jusheng_shiyue.isIn() && target) {
                                        if ([player.storage.xingxu_shiyue, player].contains(target) && get.attitude(player, player.storage.jusheng_shiyue))
                                            return [1, 0.5];
                                    }
                                }
                            }
                        ...
                        mod: {
                            aiOrder(player, card, num) {
                                if (get.attitude(player, player.storage.xingxu_shiyue) > 1 && player.storage.xingxu_shiyue2) {
                                    if (player.storage.xingxu_shiyue2.contains(card)
                                        && player.getHistory('useCard', evt => player.storage.xingxu_shiyue2.contains(evt.card)).length == 1) {
                                        if (player.needsToDiscard())
                                            return num - 2;
                                        else
                                            return num - 8;
            ...
            qingsui: {
                ...
                filter(event, player) {
                    if (event.name == 'useCard')
                        return event.skill == 'qingsui_jiai_backup';
                    return true;
                },
                locked: true,
                direct: true,
                        ...
                        filter(event, player) {
                            return get.type(event.card) == 'basic' && event.cards.length == 1 && player.getHistory('lose', evt => {
                                if (evt.getParent() != event)
                                    return false;
                                for (let i in evt.gaintag_map) {
                                    if (evt.gaintag_map[i].contains('gouhun'))
                                        return true;
                                }
                                return false;
                            }).length > 0;
            ...
            tingzhu: {
                ...
                priority: 199,
                filter(event, player) {
                    return event.getParent().type == 'card' && game.hasPlayer(cur => !event.getParent().targets.contains(cur));
                },
                direct: true,
                content() {
                    'step 0';
                    let types = get.type3(_status.discarded);
                    let check = game.hasPlayer(cur => !trigger.getParent().targets.contains(cur) && get.damageEffect(cur, player, player) > 0);
            ...
            ganfen: {
                ...
                check(event, player) {
                    if (['phaseDraw', 'phaseUse'].contains(event.name) || player.hp <= 1)
                        return false;
                    if (event.name == 'phaseJudge' && player.countCards('j') > 1)
                        return true;
                ...
                content() {
                    ...
                    if (event.target.isIn()) {
                        ...
                        event.target.chooseCard(event.gainnum, 'he', '将' + get.cnNumber(event.gainnum) + '张牌交给' + get.translation(player)).set('ai', card => {
                            ...
                            if (get.recoverEffect(player, event.target, event.target) > 0)
                                return 1;
                            if (player.isHealthy() || get.attitude(player, event.target) <= 0)
                                return -1;
                            return 0;
                        }());
                    }
                ...
                content() {
                    ...
                    player.judge(card => {
                        ...
                        if (!evt || evt.name != 'shengyang')
                            return;
                        if (event.judgeResult.number <= evt.gainnum)
                            player.gainPlayerCard([1, num], evt.target, true);
                        else
                            evt.target.recover();
                    });
                },
            ...
            shengyin: {
                ...
                content() {
                    ...
                    next.set('callback', function () {
                        ...
                        if (get.type2(event.judgeResult.name) == type2)
                            source.gain(card0, player, 'give');
                        if (event.judgeResult.color == color)
            ...
            xiuyou: {
                ...
                content() {
                    ...
                    'step 1';
                    if (result.bool) {
                        event.num = player.storage.xiuyou.contains(event.target) ? 2 : 1;
            ...
            zhepie: {
                ...
                content() {
                    ...
                    }).set('card', event.card).set('createDialog', ['『折撇』：令一名角色获得此牌',
                        [[event.card], 'card']]);
                    'step 2';
                    if (result.bool) {
                        event.target = result.targets[0];
                        ...
                        if (!event.target.storage.zhepie_cardDisable)
                            event.target.storage.zhepie_cardDisable = [];
            ...
            chumo: {
                ...
                content() {
                    ...
                    player.chooseTarget().set('ai', function (target) {
                        ...
                        return num;
                    }).set('cards', cards).set('createDialog', [get.prompt('chumo'),
                        [cards, 'card']]);
                    'step 1';
                    if (result.bool) {
                        event.target = result.targets[0];
                    ...
                    changeBy: {
                        ...
                        content() {
                            ...
                            for (let i of trigger.targets) {
                                if (get.effect(i, event.card, event.target, player) < 0)
                                    check++;
                            }
                            if (check < event.sujian.length)
                                check = 0;
            ...
            gongni: {
                ...
                animationColor: 'yami',
                filter(event, player) {
                    if (event.name != 'phaseZhunbei' && _status.currentPhase == player)
                        return false;
            ...
            lianyin: {
                ...
                priority: 996,
                filter(event, player) {
                    if (event.name == 'respond' && !player.awakenedSkills.contains('guixiang'))
                        return false;
            ...
            xuanying: {
                ...
                priority: 996,
                filter(event, player) {
                    if (event.name == 'respond' && !player.awakenedSkills.contains('houfan'))
                        return false;
            ...
            yuanjiu: {
                ...
                filter(event, player) {
                    ...
                    return esuits.length && player.countDiscardableCards(player, 'he', card => esuits.contains(card));
                },
                content() {
                    'step 0';
                    event.target = trigger.player;
                    let suits = get.suit3(event.target.getCards('e'));
                    player.chooseCard('he', get.prompt2('yuanjiu'), card => _status.event.suits.contains(get.suit(card))).set('suits', suits).set('ai', card => {
            ...
            qijian: {
                ...
                content() {
                    ...
                    'step 1';
                    if (result.bool) {
                        if (!player.hasHistory('sourceDamage', evt => evt.card.cardid == result.card.cardid && result.targets.contains(evt.player))) {
                            player.draw();
            ...
            yizhan: {
                    ...
                    count: {
                        ...
                        filter(event, player) {
                            ...
                            if (!event.player.isDying())
                                return false;
                            if (event.player.storage.yizhan_mark != undefined)
                                return false;
                            return true;
                        },
                        content() {
                ...
                filter(event, player) {
                    if (event.player.storage.yizhan)
                        return false;
                    if (event.player.isDying())
                        return false;
                ...
                content() {
                    ...
                    player.chooseControl('dialogcontrol', ['A.于摸牌阶段多摸1张牌', 'B.于出牌阶段多出1张【杀】', 'C.于弃牌阶段手牌上限+1']).set('ai', function () {
                        ...
                        if (player.storage.shangsheng[0] == -1)
                            ...
                            if (controls.contains('B.于出牌阶段多出1张【杀】') && player.countCards('hs', 'sha') >= 2 && player.hasUseTarget({ name: 'sha', isCard: true }))
                    ...
                    if (player.storage.shangsheng[0] >= 0 && player.storage.shangsheng[0] != event.map[event.change]
                        && player.storage.shangsheng[1] >= 0 && player.storage.shangsheng[1] != event.map[event.change])
            ...
            zhanchong: {
                trigger: { player: 'loseEnd' },
                filter(event, player) {
                    if (event.getParent().name && ['useCard', 'addJudge'].contains(event.getParent().name))
                        return false;
                    if (!event.visible)
                        return false;
                    for (let i = 0; i < event.hs.length; i++) {
                        if (get.type(event.hs[i]) == 'equip')
                            return true;
                    }
                    return false;
                },
            ...
            nisheng: {
                ...
                filter(event, player) {
                    ...
                    return player.countCards('h', card1 => {
                        ...
                        if (player.getStorage('nisheng').contains(num))
                            return false;
            ...
            shengang: {
                ...
                subSkill: {
                    judge: {
                        ...
                        filter(event, player) {
                            if (!game.filterPlayer(cur => get.distance(player, cur, 'pure') == 1, [player]).contains(event.player))
                                return false;
            ...
            shenghuo: {
                ...
                content() {
                    ...
                    player.chooseButton([0, Infinity], true, ['『圣火』：按顺序选择置于牌堆另一端的牌（先选择的在外侧）', '牌堆顶', [event.topCards, 'card'], '牌堆底', [event.bottomCards, 'card']]).set('ai', function (button) {
                        ...
                        let judge = next.getCards('j')[ui.selected.buttons.filter(buttonx => bottomCards.contains(buttonx.link)).length];
                        if (judge) {
                    ...
                    if (result.bool && result.links) {
                        ...
                        for (let i = 0; i < links.length; i++) {
                            if (event.topCards.contains(links[i]))
                                bottom.push(links[i]);
                            if (event.bottomCards.contains(links[i]))
                                top.unshift(links[i]);
                        }
            ...
            haosun: {
                    ...
                    switch (result.control) {
                        ...
                        case '声明一种你可以使用的基本牌并令你不能使用之，然后你本回合每次摸牌额外摸一张': {
                            player.chooseControl(get.inpile('basic', card => lib.filter.cardEnabled({ name: card }, player, 'forceEnable'))).set('prompt', '声明一种你可以使用的基本牌并令你不能使用之').set('choice', get.inpile('basic', card => {
                                ...
                                if (controls.contains('qi'))
                                    return 'qi';
                                if (controls.contains('tao') && player.hp >= 2)
                                    return 'tao';
                                if (controls.contains('jiu'))
                                    return 'jiu';
                                return controls.randomGet();
                            });
                            break;
                ...
                mod: {
                    cardEnabled(card, player) {
                        if (player.getStorage('haosun').contains(get.name(card)))
                            return false;
                    },
                    cardSavable(card, player) {
                        if (player.getStorage('haosun').contains(get.name(card)))
            ...
            banmao: {
                ...
                mod: {
                    cardEnabled(card, player) {
                        if (['shan', 'jiu'].contains(get.name(card)) && player.isHealthy() && player.storage.banmao !== true)
                            return false;
                    },
                    cardSavable(card, player) {
                        if (['shan', 'jiu'].contains(get.name(card)) && player.isHealthy() && player.storage.banmao !== true)
            ...
            kuangbiao: {
                ...
                filter(event, player) {
                    return get.suit(event.card) == 'heart' && player.hasHistory('lose', evt => {
                        if (evt.getParent() != event)
                            return false;
                        if (JSON.stringify(evt.hs) == JSON.stringify(event.cards))
                            return true;
                        return false;
                    });
                },
                ...
                content() {
                    ...
                    if (player.countMark('leizhu') == 3) {
                        ...
                        if (info.allowMultiple == false || !trigger.targets
                            || !game.hasPlayer(cur => !trigger.targets.contains(cur) && lib.filter.targetEnabled2(card, player, cur)))
                            event.finish();
                    }
                    else
                        event.finish();
            ...
            chengzhang: {
                ...
                content() {
                    ...
                    if (result.bool && ((_a = result.targets) === null || _a === void 0 ? void 0 : _a.length)) {
                        ...
                        if (target.hasUseTarget(event.cards[event.count]))
                            target.chooseUseTarget(event.cards[event.count], true);
                    }
                    'step 3';
                    if (event.count)
                        event.goto(1);
            ...
            mengdong: {
                ...
                ai: {
                    ...
                    effect: {
                        player(card, player, target, current) {
                            if (target && !player.getStorage('mengdong').contains(target) && target.countCards('e') % 2 == 1)
                                return [1, 1];
                        }
                    }
                }
            ...
            zouyang: {
                ...
                filter(event, player) {
                    ...
                    if (event.targets && event.targets.length == 1 && !info.multitarget) {
                        if (game.hasPlayer(cur => !event.targets.contains(cur) && get.distance(event.targets[0], cur, 'pure') == 1)) {
                            return true;
                        }
                    }
                    return false;
                ...
                logTarget(event, player) {
                    return game.filterPlayer(cur => !event.targets.contains(cur) && get.distance(event.targets[0], cur, 'pure') == 1);
                },
                check(event, player) {
                    return game.hasPlayer(cur => !event.targets.contains(cur) && get.distance(event.targets[0], cur, 'pure') == 1 && get.effect(cur, event.card, player, player) > 0);
                },
                content() {
                    'step 0';
                    event.draws = [];
                    event.targets = game.filterPlayer(cur => !trigger.targets.contains(cur) && get.distance(trigger.targets[0], cur, 'pure') == 1);
                    for (let i of event.targets) {
                        if (lib.filter.targetEnabled2(trigger.card, player, i)) {
                            if (event._zouyang_tmp && event._zouyang_tmp != 'target')
                                event._zouyang_tmp = 'goon';
                            else
                                event._zouyang_tmp = 'target';
                        }
                        else {
                            event.draws.add(i);
                            if (event._zouyang_tmp && event._zouyang_tmp != 'draw')
                                event._zouyang_tmp = 'goon';
                            else
                                event._zouyang_tmp = 'draw';
                        }
                    }
            ...
            wenzhou: {
                ...
                filter(event, player) {
                    if (event.name == 'damage')
                        return event.num > 1 && event.source && event.source.isIn();
            ...
            jichu: {
                mod: {
                    selectTarget(card, player, range) {
                        ...
                        if ((evt === null || evt === void 0 ? void 0 : evt.card) && get.type2(evt.card) == 'trick' && !['delay', 'equip'].contains(get.type(card)))
                            range[1] += 1;
                    },
                    aiOrder(player, card, num) {
                ...
                content() {
                    ...
                    if (list.length) {
                        let next = player.chooseControl('dialogcontrol', list).set('ai', function () {
                            let evt = _status.event.getParent();
                            let controls = _status.event.controls.slice(0);
                            if (evt.discards.length >= 4 && controls.contains('获得本回合进入弃牌堆的任意类型不同的牌，且若这些牌之和为质数，令其回复1点体力'))
                                return 0;
            ...
            minghuahongxiao: {
                ...
                check(event, player) {
                    if (event.name == 'useCard' && player.isPhaseUsing() && player.countCards('h') && get.type2(event.card) == 'trick')
                        return false;
                    return true;
                },
                content() {
                ...
                subSkill: {
                    change: {
                        ...
                        content() {
                            'step 0';
                            if (['equip', 'delay'].contains(get.type(trigger.card))
                                || !game.hasPlayer(cur => !trigger.targets.contains(cur) && lib.filter.targetEnabled2(trigger.card, player, cur)))
                                event.goto(4);
                            'step 1';
                            let prompt2 = '为' + get.translation(trigger.card) + '增加一个目标';
            ...
            yikai: {
                ...
                content() {
                    ...
                    'step 3';
                    if (result.control !== 'cancel2') {
                        if (target.awakenedSkills.contains(result.control)) {
                ...
                subSkill: {
                    ...
                    blocker: {
                        ...
                        onremove(player, skill) {
                            delete player.getStorage('yikai_blocker');
                    ...
                    result: {
                        target(player, target) {
                            if (target.hp > 1) {
                                let skills = target.getOriginalSkills();
                                for (let i of skills) {
                                    if (lib.skill[i].limited && target.awakenedSkills.contains(i)) {
                                        return 8;
                                    }
                                }
                            }
            ...
            qinhuo: {
                ...
                filter(event, player) {
                    if (event.cards && get.name(event.card) == 'huogong'
                        && !event.player.hasHistory('sourceDamage', evt => event == evt.getParent('useCard')))
                        return true;
                },
                content() {
                    'step 0';
            ...
            yangxi: {
                ...
                check(card) {
                    let player = _status.event.player;
                    if (['shandian', 'fulei', 'haidi'].contains(get.name(card)))
                        return 9 - get.value(card);
                ...
                ai: {
                    ...
                    result: {
                        player(player, target) {
                            if (player.countCards('h', card => ['shandian', 'fulei', 'haidi'].contains(get.name(card))))
                                return 0;
                            return -1.5;
                        },
                        target(player, target) {
            ...
            wuneng: {
                enable: 'phaseUse',
                filter(event, player) {
                    return player.countCards('h', card => ['tao', 'taoyuan'].contains(card.name) && !card.hasGaintag('ming_'));
                },
                filterCard(card, player) {
            ...
            tage: {
                ...
                filter(event, player) {
                    ...
                    return typeof num == "number" && player.countCards('hs', card => [1, -1].contains(get.number(card) - num));
                },
                content() {
                    'step 0';
            ...
            aowei: {
                ...
                filter(event, player) {
                    ...
                    if (evt.name != 'orderingDiscard' || !evt.relatedEvent || evt.relatedEvent.player == player
                        || !['useCard', 'respond'].contains(evt.relatedEvent.name) || get.name(evt.relatedEvent.card) != 'sha')
                        return false;
