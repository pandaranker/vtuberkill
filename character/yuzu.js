...
game.import('character',function(lib,game,ui,get,ai,_status){
	return {
		...
		character:{
			...
			/**永雏塔菲 */
			Taffy:['female','qun',3,['qianqi','chutan']],
			/**谢拉 */
		...
		//		prepare:'give',
			...
			echi:{
				...
				content:function(){
					...
					player.chooseCard(get.prompt2('echi',event.tar)).set('logSkill',['echi',event.tar]).set('ai',function(card){
				...
				content:function(){
					...
					if(!trigger.huanxi){
						...
						if(trigger.huanxi.length+names.length==trigger.huanxi.addArray(names).length){
							if(player.hasSkill('celv_cardDisable'))	player.removeSkill('celv_cardDisable');
						}
					}
				}
			},
					...
					cardDisable:{
						...
						mod:{
							cardEnabled2:function(card,player){
								return player.storage.celv_cardDisable.contains(get.name(card));
							},
						},
					}
				},
			...
			xueta:{
				...
				content:function(){
					...
					player.chooseToDiscard('he',get.prompt2('xueta')).set('ai',function(card){
						if(!_status.event.check)	return -get.value(card);
						return 8-get.value(card);
				...
				content:function(){
					var func=function(result){
						...
						if(get.subtype(result)=='equip1'){
							num+=get.value(result,'raw',player)/2;
						}
			...
			akjianwu:{
				...
				content:function(){
					...
					if(result.winner&&result.loser){
						...
						var check = 1;
						if(event.card.cards&&get.value(event.card.cards,'raw',event.winner)>event.loser.countGainableCards(event.winner,'he'))	check = 0;
			...
			dianying:{
				...
				content:function(){
					...
					next.set('ai',function(button){
						var player = _status.event.player;
						var target = _status.event.target;
						return get.attitude(player,target)<=0||get.value(button.link,'raw',player)>get.value(button.link,'raw',target);
					});
					'step 1'
					if(result.bool&&result.links){
		...
		translate:{
			...
			Ironmouse_ab: '铁耗子',
			haosun: '耗损',
			haosun_info: '回合开始时，你可以选择一项：<br>回复1点体力以重置此技能并修改『伴猫』，然后你本回合每次摸牌少摸一张；声明一种你可以使用的基本牌并令你不能使用之，然后你本回合每次摸牌额外摸一张。',
			banmao: '伴猫',
			...
			xiaoxiayu_ab: '小虾鱼',
			tanghuang: '堂皇',
			tanghuang_info: '当你成为其他角色使用牌的目标时，你可以摸X张牌并令其弃置你和其共计X+3张牌，然后弃置牌数：较少者～受到一点伤害；较多者～摸弃牌差的牌。（X为你已损失的体力值且至少为1）',
			xiejiang: '蟹酱',
			xiejiang_info: '锁定技 你摸两张以上的牌后，获得1点护甲；你失去1点护甲后，当前回合角色摸两张牌。',

			tianxixi: '田汐汐',
			lache: '拉扯',
			lache_info: '你回复体力时，可以令当前回合角色摸两张牌；你弃置两张以上的牌或护甲减少后，可以回复一点体力，若发生在回合外，你摸等量牌。',
			danfu: '蛋孵',
			danfu_info: '锁定技 结束阶段，若你本回合未造成伤害，你失去一点体力并获得1点护甲；你失去1点护甲后，当前回合角色摸两张牌。',
			...
			RinaHayashi: '林莉奈',
			xilv: '习律',
			xilv_info: '其他角色因为技能摸牌时，你可以交给其一张牌，然后其选择一项：<br>1.交给你摸到的牌；2.若你没有对应技能，令你获得之，直到你的下个回合结束。',
			bana: '拔奈',
			...
			xueta: '靴踏',
			xueta_info: '当你响应其他角色使用的牌后，若其不是皇珈骑士，你可以弃一张牌，令其摸两张牌并成为<皇珈骑士>。',
			yuezhi: '乐治',
			yuezhi_info: '<font color=#a7f>觉醒技</font> 回合开始时，若场上<皇珈骑士>的数量大于你的体力值或手牌数，你增加一点体力上限并从弃牌堆获得你的初始手牌，每有一张无法获得，你回复1点体力并摸两张牌，然后修改（）内容为“你或一名<皇珈骑士>”。',
			...
			ysxiangxing_append:'<span style="font-family: LuoLiTi2;color: #dbb">特性：易上手</span>',
			...
			Naraka : '奈罗花',
			echi: '阿斥',
			echi_info: '其它角色于摸牌阶段外获得牌时，若该角色的体力值不小于你，你可展示一张手牌并令其选择一项：<br>弃置一张同种类的牌；失去一点体力。',
			mudu: '哞督',
