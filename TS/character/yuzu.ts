window.game.import('character', function (lib, game, ui, get, ai, _status) {
	...
	return <currentObject>{
		...
		character: {
			...
			/**凤玲天天 */
			HoureiTenten: ['female', 'qun', 3, ['shengquan', 'yizhu'],],
		...
		skill: {
			...
			tatongling: new toSkill('trigger', {
				...
				check(Evt, player) {
					...
					if (Evt.player.isTurnedOver()) return get.attitude(player, Evt.player) > 0
			...
			lache: {
				...
				check(Evt, player) {
					if (Evt.name == 'recover') return get.attitude(player, _status.currentPhase) > 0;
					else return true;
				},
				content() {
			...
			tiantang: {
				...
				filter(Evt, player) {
					if (player.countCards('he') < (Evt.player.storage.paryi || 1)) return false;
					return true;
				},
				check(Evt, player) {
					if (player.storage.haoren !== true) return (Evt.player.storage.paryi || 1) <= 2 && get.attitude(player, Evt.player) < 1 && !Evt.player.hasJudge('lebu');
			...
			huange: {
				...
				check(Evt, player) {
					if (Evt.player != player && get.attitude(player, Evt.player) < 0 && Evt.player.inRange(player)) return true;
				...
				subSkill: {
					date: {
						...
						filter(Evt, player) {
							...
							if (player.storage.qishi_date && player.storage.qishi_date.includes(Evt.player)) return false;
							return true;
						},
						content() {
			...
			niaoji: {
				...
				lastDo: true,
				check(Evt, player) {
					if (Evt.source && Evt.source == player) return get.attitude(player, Evt.player) < 1;
					return true;
				},
				frequent: true,
				prompt(Evt, player) {
					if (Evt.source && Evt.source == player) return `对${get.translation(Evt.player)}造成伤害，${get.prompt('niaoji')}`;
			...
			tiaolian: {
				...
				check(Evt, player) {
					...
					if (Evt.player != player) return get.attitude(player, Evt.player) < 1;
				},
				content() {
					'step 0'
			...
			tuncai: {
				...
				round: 1,
				filter(Evt, player) {
					if (Evt.name == 'draw') return Evt.player != player && player.storage.tuncai;
					else return Evt.cards.length && !player.storage.tuncai;
				},
				check(Evt, player) {
					if (player.storage.tuncai == true) return Evt.num >= 2;
					return true;
				},
				popup: false,
				content() {
					'step 0'
					if (trigger.name == 'draw') {
						player.logSkill('tuncai', player); player.draw(trigger.num); player.storage.tuncai = !player.storage.tuncai; Evt.finish();
			...
			mianmo: {
				...
				content() {
					...
					if (result.bool) {
						...
						if (player.storage.tiaolv_up && player.storage.tiaolv_up.includes(Evt.card)) Evt.goto(4);
						if (player.storage.tiaolv_down && player.storage.tiaolv_down.includes(Evt.card)) Evt.goto(6);
					} else {
						Evt.finish();
					}
					'step 2'
			...
			tiaolv: {
				...
				subSkill: {
					up: {
						...
						priority: 5,
						content() {
							if (player.storage[Evt.name].length) player.storage[Evt.name].length = 0;
						},
						mod: {
							number(card, player, number) {
			...
			ming_niwei: {
				...
				priority: 3,
				filter(Evt, player) {
					if (Evt.player.hasSkill('niwei') && Evt.player.storage.niwei && Evt.player.storage.niwei.includes(Evt.name)) return true;
					if (Evt.player != player) return false;
				...
				subSkill: {
					mark: {
						...
						content() {
							...
							'step 1'
							if (Evt.list[Evt.num]) {
								if (player.storage.shixi_mark) Evt.record.removeArray(player.storage.shixi_mark);
			...
			zhengniu: {
				...
				content() {
					...
					let check = get.attitude(player, Evt.target) > 0,
						list1 = player.storage.juehuo.ans.slice(0), list2 = player.storage.juehuo.ms.slice(0);
					let list: Dialogword = ['###' + get.prompt('zhengniu') + '###选择交给' + get.translation(Evt.target) + '的绝活'];
					if (list1.length) {
						list.push('暗置绝活');
						list.push([list1, 'card']);
					}
						...
						content() {
							...
							if (Evt.current == player) {
								...
								if ((Evt.current == game.me && !_status.auto) || (
									get.attitude(Evt.current, player) > 2) ||
				...
				content() {
					...
					if (Evt.guessers.length == 0) Evt.goto(3);
						...
						Evt.guessers[0].chooseControl('质疑', '不质疑').set('prompt', Evt.prompt).set('ai', function () {
							if (get.attitude(Evt.guessers[0], player) > 0) return '不质疑';
			...
			ganfen: {
				...
				subSkill: {
					fanmian: {
						...
						filter(Evt, player) {
							if (player.storage.ganfen_clickChange === false) return false;
							if (player.storage.mian && player.storage.mian.ans.length) {
				...
				content() {
					...
					if (Evt.target.isIn()) {
						...
						Evt.target.chooseCard(Evt.gainnum, 'he', '将' + get.cnNumber(Evt.gainnum) + '张牌交给' + get.translation(player)).set('ai', card => {
							...
							if (player.isHealthy() || get.attitude(player, Evt.target) <= 0) return -1;
							return 0;
						}());
					} else Evt.finish();
					'step 2'
			...
			daoyi: {
				...
				content() {
					...
					if (result.bool == true) {
						...
						if (lib.skill.daoyi.map[player.storage.daoyi] == 'number') trigger.fixedResult[lib.skill.daoyi.map[player.storage.daoyi]] = lib.number.indexOf(link) + 1;
						else trigger.fixedResult[lib.skill.daoyi.map[player.storage.daoyi]] = link;
			...
			zhepie: {
				...
				content() {
					...
					if (result.bool) {
						...
						if (!Evt.target.storage.zhepie_cardDisable) Evt.target.storage.zhepie_cardDisable = [];
			...
			yizhan: {
				subSkill: {
					count: {
						...
						filter(Evt, player) {
							...
							if (Evt.player.storage.yizhan_mark != undefined) return false;
							return true;
						},
						content() {
							trigger.yizhan = true;
				...
				filter(Evt, player) {
					if (Evt.player.storage.yizhan) return false;
					if (Evt.player.isDying()) return false;
			...
			shangsheng: {
				...
				content() {
					...
					if (player.storage.shangsheng[0] >= 0 && player.storage.shangsheng[0] != Evt.map[Evt.change]
						&& player.storage.shangsheng[1] >= 0 && player.storage.shangsheng[1] != Evt.map[Evt.change]) player.storage.shangsheng_Buff++;
			...
			shengang: {
				...
				subSkill: {
					judge: {
						...
						filter(Evt, player) {
							if (!game.filterPlayer(cur => get.distance(player, cur, 'pure') == 1, [player]).includes(Evt.player)) return false;
			...
			shengquan: new toSkill('trigger', {
				...
				content() {
					'step 0'
					player.chooseToDiscard([2, Infinity], 'h', get.prompt2('shengquan')).set('complexCard', true).set('filterCard', (card, player) => {
						if (ui.selected.cards.length) return get.color(card) == get.color(ui.selected.cards[0]);
						return player.countCards('hes', { color: get.color(card) }) >= 2;
			...
			yizhu: new toSkill('active', {
				...
				content() {
					...
					'step 2'
					player.storage.yizhu_giveBy = target
				...
				ai: {
					...
					result: {
						player(player, target) {
							...
							if (player.storage.yizhu === target && get.attitude(player, target) > 0) return -1
							return player.hp - 2
						},
						target(player, target) {
							if (player.hp === 1) return 0
							if (player.storage.yizhu === target) return -1
							return 2;
						}
					},
					threaten: 0.6
				...
				subSkill: {
					giveBy: new toSkill('trigger', {
						filter(Evt, player) {
							return player.storage.yizhu_giveBy?.isIn() && (Evt.cards2 || Evt.cards).filterInD('d').length
						},
						logTarget(Evt, player) {
							return player.storage.yizhu_giveBy
						},
						content() {
							Evt.tar = player.storage.yizhu_giveBy
