...
mixin(game, /**@lends module:core.game */ {
    ...
    /**
        ...
        */
    putDB: function (type:string, id:string, item:any, callback?:(this:IDBRequest<IDBValidKey>, ev:Event)=>void):void {
        ...
        put.onsuccess = function (ev:Event) {
            if (callback) {
    ...
    /**
        ...
        * @param {!function} callback - 成功时回调函数
        */
    getDB: function (type:string, id:string | null, callback:(this:void, item:any | null)=>void):void {
        if (!lib.db) {
            callback(null);
            return;
        }
        ...
        if (id) {
            store.get(id).onsuccess = function (e) {
                _status.dburgent = true;
                callback((e.target as IDBRequest<any>).result);
                delete _status.dburgent;
                game.reload2();
            };
        }
            ...
            store.openCursor().onsuccess = function (e) {
                var cursor = (e.target as IDBRequest<IDBCursorWithValue | null>).result;
                if (cursor) {
                    obj[cursor.key as any] = cursor.value;
                    cursor.continue();
                }
                else {
                    _status.dburgent = true;
    ...
    /**
        ...
        * @param {function} callback - 成功时回调函数
        */
    deleteDB: function (type:string, id:string, callback:(this:IDBRequest<undefined> | void, ev:Event | false)=>void) {
        if (!lib.db) {
            callback(false);
            return;
        }
        ...
        if (arguments.length == 1) {
            game.getDB(type, null, function (obj: any | null) {
                var store = lib.db!.transaction([type], 'readwrite').objectStore(type);
                for (var id in obj) {
                    lib.status.reload++;
                }
            ...
            lib.status.reload++;
            var store = lib.db.transaction([type], 'readwrite').objectStore(type);
            store.delete(id).onsuccess = function (ev:Event) {
                if (callback) {
                    callback.call(this, ev);
                }
                game.reload2();
    ...
    /**
        ...
        */
    saveConfig: function (key: string, value?: any, local?: string, callback?: () => void) {
        if (_status.reloading) return;
        if (local) {
            var localmode;
