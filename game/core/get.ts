...
mixin(get, /**@lends module:core.get */ {
    ...
    is: {
        ...
        banWords: function (str) {
            ...
            for (let i of (window as any).bannedKeyWords) {
        ...
        while (num--) {
            if (ui.cardPile.hasChildNodes() == false) {
                ...
                for (let i = 0; i < ui.discardPile.childNodes.length; i++) {
                    let currentcard = ui.discardPile.childNodes[i];
            ...
            let cardx = ui.cardPile.removeChild(ui.cardPile.lastChild);
            cardx.original = 'c';
            list.push(cardx);
        }
    ...
    ip: function () {
        if (localStorage.getItem('noname_inited') !== 'nodejs') return '';
    ...
    skillState: function (player:PlayerModel) {
        let skills = {
            stat:{},
            skillinfo:{},
     ...
     * @param item 配置项-键值
     * @param mode 游戏模式名
     * @returns {string|boolean|undefined|null}
     */
    config: function (item: string, mode: string = lib.config.mode): any {
    ...
    infoFuncOL: function (info: string) {
        const {_status, lib, game, ui, get, ai} = _context;
        var func;
        try {
    ...
    equiptype: function (card, player:PlayerModel) {
    ...
    suit(card, player:PlayerModel):string {
    ...
    suit3(cards, player:PlayerModel):string[] {
        if (get.itemtype(cards) != 'cards') return [];
        var suits = [];
        for (let i of cards) {
    ...
    color(card, player:PlayerModel):string {
        if (_status.event.name == 'judge' && card.color) return card.color;
        if (get.itemtype(card) == 'cards') {
            let color = get.color(card[0], player)
    ...
    color3(cards, player:PlayerModel):string[] {
        if (get.itemtype(cards) != 'cards') return;
        var colors = [];
        for (let i of cards) {
     ...
     */
    distance(from:PlayerModel, to:PlayerModel, method) {
        if (from == to) return 0;
     ...
     */
    //TODO
    position(card, ordering?:boolean):string|number {
    ...
    skillTranslation: function (str, player:PlayerModel):string {
        var str2, str3;
    ...
    interoperableText(name, player:PlayerModel):[string,Object] {
    ...
    cnNumber(num, two):string|number {
    ...
    skillintro: function (name, learn, learn2) {
        ...
        if (!infoitem) {
            for (let itemx in lib.characterPack) {
     ...
     */
    nodeintro: function (node, simple, evt):HTMLDivElement {
        ...
        if (typeof node._customintro == 'function') {
            ...
            let skills = node.getSkills(null, null, false).slice(0);
            let skills2 = game.filterSkills(skills, node);
                ...
                if (lib.translate[skills[i] + '_info']) {
                    ...
                    if (node.forbiddenSkills[skills[i]]) {
                        ...
                        if (lib.skill[skills[i]].frequent || lib.skill[skills[i]].subfrequent) {
                            uiintro.add('<div><div class="skill thundertext thunderauto">' + translation + '</div><div' + ((translation.length > 3) ? ' class="skilltext thundertext thunderauto"' : ' class="thundertext thunderauto"') + '>' + get.skillInfoTranslation(skills[i], node) + '<br><div class="underlinenode on gray" style="position:relative;padding-left:0;padding-top:7px">自动发动</div></div></div>');
                        ...
                        uiintro.add('<div><div class="skill">' + translation + '</div><div' + ((translation.length > 3) ? ' class="skilltext"' : '') + '>' + get.skillInfoTranslation(skills[i], node) + '<br><div class="underlinenode on gray" style="position:relative;padding-left:0;padding-top:7px">自动发动</div></div></div>');
                        var underlinenode = uiintro.content.lastChild.querySelector('.underlinenode');
                        if (lib.skill[skills[i]].frequent) {
                            if (lib.config.autoskilllist.contains(skills[i])) {
                                underlinenode.classList.remove('on');
            ...
            if (!game.observe && _status.gameStarted && game.me && node != game.me) {
                ...
                var click = function () {
                    ...
                    setTimeout(function () {
                        ...
                        if (ui.throwEmotion) {
                            for (let i of ui.throwEmotion) i.classList.remove('exclude');
                        }
            ...
            if (!simple || get.is.phoneLayout()) {
                if ((lib.config.change_skin || lib.skin) && !node.isUnseen()) {
                    ...
                    let createButtons = function (num, avatar2?) {
                        if (!introadded) {
                            introadded = true;
                    ...
                    let loadImage = function (avatar2?) {
                        let img = new Image();
                        img.onload = function () {
                            num++;
                            loadImage(avatar2);
            ...
            if (node._banning) {
                ...
                var banall = false;
                for (let i = 0; i < list.length; i++) {
                    let cfg = ui.create.div('.config', lib.translate[list[i]] + '模式', page);
            ...
            if (node._banning) {
                ...
                if (addskin && (!simple || get.is.phoneLayout())) {
                    ...
                    let createButtons = function (num) {
                        if (!num) return;
                        if (!introadded) {
                            introadded = true;
                            uiintro.add('<div class="text center">更改皮肤</div>');
