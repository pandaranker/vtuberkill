...
mixin(lib, /**@lends module:core.lib */ {
    ...
    characterDialogGroup: {
        '收藏': function (name: string, capt: string) {
            return lib.config.favouriteCharacter.contains(name) ? capt : null;
        },
        '最近': function (name: string, capt: string) {
     ...
     */
    listenEnd: function (node: PlayerDivElement | HTMLDivElement) {
    ...
    cheat: {
        ...
        u: function () {
            var card:cardArray = { name: 'sha' }, source = game.me.next, targets = [];
            for (var i = 0; i < arguments.length; i++) {
                if (get.itemtype(arguments[i]) == 'player') {
                    source = arguments[i];
                }
    ...
    message: {
        ...
        client: {
            ...
            updaterooms: function (list, clients) {
                if (ui.rooms) {
                    let map = {}, map2 = {};
                    for (let i of ui.rooms) map2[i.key] = true;
                    for (let i of list) {
                        if (!i) continue;
                        map[i[4]] = i;
                    }
                ...
                var proceed = function () {
                    game.loadModeAsync(config.mode, function (mode) {
                        ...
                        if (mode.element && mode.element.player) {
                            mixin(PlayerModel.prototype, mode.element.player);
                        }
                        if (mode.skill) {
