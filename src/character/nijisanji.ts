				...
				subSkill: {
					phaseUse: {
						...
						content() {
							...
							// player.storage.yinlaiyaotang_phaseUse = player.storage.yinlaiyaotang_phaseUse.concat(Evt.prepareCards);
					...
					loseCheck: {
						...
						filter(Evt, player) {
							...
							// 			if(player.storage.yinlaiyaotang_phaseUse[j].name == Evt.cards[i].name){
							// 				//保存检索的位置信息
