...
export default {
    ...
    yanwang: `妄诳`,
    yanwang_info: `其他角色使用牌指定你为目标时，其可以令你回复一点体力，然后展示并获得你的一张手牌；若其因此获得了黑色牌，你可以令其视为对你指定的一名角色使用一张【决斗】。`,
    ...
    chutan_info: `出牌阶段限一次，你可以弃置一张牌，标记一名其他角色。<br>
    被『雏探』标记角色的回合结束时，若其于本回合使用牌指定了另一名被标记角色为目标，你获得其本回合使用的实体牌，然后移除其标记状态。`,
    chutan_append: lib.figurer(`其他人看不见『雏探』标记了哪位角色`),
    ...
    liangshan: `汉歌`,
    liangshan_info: `其他角色在你的回合内首次摸牌后，你可以将牌堆顶牌置于你的武将牌上。一名角色回合开始或濒死时，你可以交给其一张『汉歌』牌，视为其使用了一张【酒】。`,
    ...
    erni: `耳匿`,
    erni_change: `耳匿`,
    erni_info: `转换技 你可以展示一张手牌并置于牌堆顶，视为使用或打出了一张同花色的①【杀】②【闪】③【桃】；结束阶段，你可以额外转换一次『耳匿』。`,
