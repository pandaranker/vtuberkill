...
export default {
    ...
    你造成或受到火焰伤害后，若伤害来源手牌/体力全场最少，其翻面并摸一张牌/回复一点体力。`,
    zouyang: `奏扬`,
    ...
    hupo: `虎迫`,
    hupo_info: `轮次技 其他角色的出牌阶段开始时，你可以对其造成X点伤害，
    因此进入濒死状态的角色，不会因本次濒死求救失败而死亡；
    若此回合结束时你不在其攻击范围内，其回复X点体力。<br>（X为你已损失的体力值）`,

    NerunaNero: `寝月ねろ`,
    peijiu: `醅酒`,
    peijiu_info: `你的【杀】指定目标时，可以对目标造成 1 点伤害，
    因此进入濒死状态的角色，不会因本次濒死求救失败而死亡。`,
    ransha: `染姹`,
    ...
    xinkeniang: `新科娘`,
    daimao: `呆毛科技`,
    daimao_info: `锁定技 游戏开始时，你将牌堆顶牌置于武将牌上，称为「萪」；你使用与「萪」同花色的牌不受距离和次数限制；你进入濒死状态时，可以将一张与「萪」不同花色的牌置为「萪」，若如此做，则你体力上限-1，回复体力至上限，摸三张牌。`,
    hongtou: `红头文件`,
    ...
    Ciyana: `希亚娜`,
    yankui: `魇窥`,
    yankui_info: `其他角色的准备阶段，你可以弃置一张与本轮以此法弃置的牌类型均不同的牌，然后观看其手牌，展示并获得其中一张。若此牌为：<br>
    非基本牌，本回合其跳过判定阶段与弃牌阶段；基本牌，本回合其可以多使用一张【杀】。`,
    ...
    kangqie: `远弦`,
    kangqie_info: `结束阶段，你可以翻面并回复一点体力，令一名角色受到一点无来源的伤害；若你于本局游戏内受到了至少7次无来源的伤害，或你体力为1，此技能也可以在出牌阶段发动。`,
    ...
    yueyinkongye: `月隐空夜`,
    zhanji: `占机`,
    zhanji_info: `当你不因此技能获得牌时，你可以弃置 3 张牌（若不足则全弃），指定{一/两}个花色，摸一张牌并展示之，若此牌为你指定的花色，你可以令{三/两}名角色摸 2 张牌；若此牌不为你指定的花色，你可以重复此流程。`,
    zhanji_append: lib.figurer(`特性：赌狗`),
    jiyang: `激扬`,
    jiyang_info: `其他角色的结束阶段，若其手牌数比你多，你可以视为对其使用一张【决斗】，若此【决斗】造成伤害，你可以将响应该【决斗】的一张牌置于牌堆顶，然后获得伤害来源一张牌。`,
