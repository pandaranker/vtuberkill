...
export default {
    ...
    re_taiyangzhiyin: {
        ...
        content: [() => {
            ...
            game.broadcastAll(function (id, choicelist) {
                let dialog = ui.create.dialog('选择一项');
            ...
            player.chooseButton().set('dialog', Evt.videoId).set('prompt', get.prompt('re_taiyangzhiyin'));
        }, () => {
    ...
    qiujin: new toSkill('trigger', {
        filter(Evt, player) {
            if (!player.hasZhuSkill('qiujin')) return false;
            return game.hasPlayer(cur => {
                return cur.getGroups('nijisanji') && cur !== player;
            });
        },
        content: [() => {
            player.chooseTarget(function (card, player, target) {
                return target.getGroups('nijisanji') && player != target;
            })
                .set('ai', (target) => get.$a2(target) > 0)
        }, () => {
            if (result.bool && result.targets) {
                Evt.target = result.targets[0]
                player.logSkill('qiujin', Evt.target)
                Evt.target.draw()
                Evt.target.link(true)
            }
        }]
    }, 'zhuSkill').setT('phaseJieshuBegin'),
    //re德龙
    re_zhenyin: {
        audio: 'zhenyin',
        trigger: {
