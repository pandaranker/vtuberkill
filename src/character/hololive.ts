...
window.game.import('character', function (lib, game, ui, get, ai, _status) {
	return {
		...
		translate: {
			...
			AkaiHaato: `赤井心`,
			liaolishiyan: `料理实验`,
			liaolishiyan_info: `摸牌阶段，你可改为展示并获得牌堆顶的两张牌，然后根据其中的花色执行对应效果：♦~重铸一张牌，♣~弃置一张牌，♥~令赤井心回复 1 点体力，♠~失去 1 点体力。<br>
			出牌阶段限一次，你可以重铸与当回合『料理实验』花色相同的两张牌令一名角色执行对应效果。`,
			liaolishiyan2: `料理实验`,
			...
			yong: `咏`,
			yemuxingyong: `夜幕星咏`,
			yemuxingyong_info: `轮次技 一个弃牌阶段结束时，你可将本阶段进入弃牌堆的牌置于武将牌上，称为「咏」；此时其他角色也可将一张黑色牌置为「咏」。<br>
			若你已觉醒『星河舰队』，你的攻击范围和手牌上限增加「咏」的数量<br><br>
			出牌阶段，你可获得一张「咏」，然后可以立即将两张手牌当【过河拆桥】或【酒】使用。`,
			xinghejianduei: `星河舰队`,
			xinghejianduei_info: `<font color=#ccf>觉醒技</font> 一轮开始时，若你的体力值不大于游戏轮数，你扣减1点体力上限并摸八张牌。`,

			SakuraMiko: `樱巫女`,
			haodu: `豪赌`,
			haodu_info: `出牌阶段限X次（X为你已损失的体力值且至少为1)，你可以将至少一张手牌交给一名其他角色并声明点数、花色、类型，然后你展示其一张手牌。根据展示牌与声明相同的项依次执行对应效果：<br>
			点数~你与其交换手牌；类型~令其弃置两张牌；花色~你获得其一张牌。			`,
