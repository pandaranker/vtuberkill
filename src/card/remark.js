		...
		skill: {
			...
			rm_qinggang_skill: {
				...
				content: function () {
					...
					trigger.target.$.rm_qinggang2.add(trigger.card);
				},
				ai: {
					unequip_ai: true,
			...
			rm_qinggang2: {
				...
				charlotte: true,
				filter: function (event, player) {
					return player.$.rm_qinggang2 && event.card && player.$.rm_qinggang2.contains(event.card) && (event.name != 'damage' || event.notLink());
				},
				silent: true,
				forced: true,
				popup: false,
			...
			rm_fangtian_guozhan: {
				...
				content: function () {
					...
					if (result.bool) {
						...
						if (!player.$.rm_fangtian_guozhan_trigger) player.$.rm_fangtian_guozhan_trigger = [];
						player.$.rm_fangtian_guozhan_trigger.add(trigger.card);
			...
			rm_fangtian_guozhan_remove: {
				...
				silent: true,
				filter: function (event, player) {
					return player.$.rm_fangtian_guozhan_trigger && player.$.rm_fangtian_guozhan_trigger.contains(event.card);
				},
				content: function () {
					player.$.rm_fangtian_guozhan_trigger.remove(trigger.card);
				}
			},
			rm_qilin_skill: {
				equipSkill: true,
			...
			rm_zhuque_skill2: {
				...
				popup: false,
				content: function () {
					delete player.$.rm_zhuque_skill.nature;
				}
			},
