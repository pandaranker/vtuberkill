module.exports = {
    getData(vkCore) {
        ...
        return {
            ...require('./lib_element_player_custom').getData(vkCore),
            //新函数
            /**
             * 将技能移入本角色的封锁列表
             */
            ...
            canSave: function (target) {
                ...
                let name = {}, hs = player.getCards('hs');
            ...
            chooseToPlayBeatmap: function (beatmap) {
                let next = game.createEvent('chooseToPlayBeatmap');
                next.player = this;
            ...
            chooseToMove: function () {
                let next = game.createEvent('chooseToMove');
                next.player = this;
            ...
            $throwEmotion: function (target, name) {
                ...
                let top = player.getTop() + player.offsetHeight / 3 - height;
                emotion.style.left = left + 'px';
                emotion.style.top = top + 'px';
                let left2 = getLeft(target) - width;
                let top2 = target.getTop() + target.offsetHeight / 3 - height;
            ...
            hasUsableCard: function (name) {
                ...
                for (let i = 0; i < skills.length; i++) {
                    ...
                    if (ifo.viewAs && typeof ifo.viewAs != 'function' && ifo.viewAs.name == name) {
                        ...
                        let hiddenCard = get.info(skills[i]).hiddenCard;
            ...
            inRange: function (to) {
                ...
                let mod1 = game.checkMod(from, to, 'unchanged', 'inRange', from);
                if (mod1 != 'unchanged') return mod1;
                let mod2 = game.checkMod(from, to, 'unchanged', 'inRangeOf', to);
                if (mod2 != 'unchanged') return mod2;
                if (from.getAttackRange() < 1) return false;
                let player = from, m, n = 1, i;
                let fxy, txy;
                if (game.chess) {
                    fxy = from.getXY();
                    txy = to.getXY();
                ...
                let equips1 = from.getCards('e', function (card) {
            ...
            getDamagedHp: function () {
                return Math.max(0, this.maxHp - Math.max(0, this.hp));
            },
            ...
            $disableEquip: function (skill) {
                ...
                let pos = { equip1: '武器栏', equip2: '防具栏', equip3: '+1马栏', equip4: '-1马栏', equip5: '宝物栏' }[skill];
                if (!pos) return;
                let card = game.createCard('feichu_' + skill, pos, '');
                card.fix();
                card.style.transform = '';
                card.classList.remove('drawinghidden');
                card.classList.add('feichu');
                delete card._transform;
                let equipNum = get.equipNum(card);
                let equipped = false;
            ...
            $disableJudge: function () {
                let player = this;
                let card = game.createCard('disable_judge', '', '');
            ...
            $enableJudge: function () {
                ...
                for (let i = 0; i < player.node.judges.childNodes.length; i++) {
            ...
            init: function (character, character2, skill) {
                ...
                let hp1 = get.infoHp(info[2]);
                let maxHp1 = get.infoMaxHp(info[2]);
                ...
                if (character2 && lib.character[character2]) {
                    let info2 = lib.character[character2];
                    if (!info2) {
                        info2 = ['', '', 1, [], []];
                    }
                    if (!info2[4]) {
                    ...
                    if ((info2[1] instanceof Array) && !this.subgroup) {
                        this.subgroup = info2[1][1];
                    }
                    let hp2 = get.infoHp(info2[2]);
                    let maxHp2 = get.infoMaxHp(info2[2]);
                    let double_hp;
                ...
                if (this.group && !lib.group.includes(this.group)) {
                    lib.group.add(this.group);
                }
                if (this.inits) {
                    for (let i = 0; i < lib.element.player.inits.length; i++) {
            ...
            flashAvatar: function (skill, name) {
                if (lib.skill[name] && !lib.character[name]) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        for (let j in lib.characterPack[list[i]]) {
            ...
            countUsed: function (card, type) {
                if (type === true) {
                    let num = 0;
                    let history = this.getHistory('useCard');
                ...
                if (arg3 !== false) {
                    for (let i = 0; i < this.node.equips.childElementCount; i++) {
                        if (!this.node.equips.childNodes[i].classList.contains('removing')) {
                            let equipskills = get.info(this.node.equips.childNodes[i], false).skills;
                            if (equipskills) {
                    ...
                    if (arg3 !== false) {
                        for (let i = 0; i < this.node.equips.childElementCount; i++) {
                            if (!this.node.equips.childNodes[i].classList.contains('removing') && !this.node.equips.childNodes[i].classList.contains('feichu')) {
                                let equipskills = get.info(this.node.equips.childNodes[i]).skills;
                                if (equipskills) {
                                    es.addArray(equipskills);
                                }
                            }
            ...
            chooseToRespond: function () {
                ...
                if (next.prompt != false) {
                    if (typeof next.prompt == 'string') {
                        ...
                        let str = '请打出' + get.cnNumber(next.selectCard[0]) + '张'
                        if (filter) {
                            if (filter.name) {
                        ...
                        if (_status.event.getParent().name == 'useCard') {
                            let cardname = _status.event.name;
            ...
            canMoveCard: function (withatt, nojudge, moveHandcard) {
                ...
                return game.hasPlayer(function (current) {
                    let att = get.sgn(get.attitude(player, current));
                    if (!withatt || att != 0) {
                        ...
                        for (let i = 0; i < es.length; i++) {
                            if (game.hasPlayer(function (current2) {
                                if (withatt) {
                                    ...
                                    let att2 = get.sgn(get.attitude(player, current2));
                    ...
                    if (!nojudge && (!withatt || att > 0)) {
                        ...
                        for (let i = 0; i < js.length; i++) {
                            if (game.hasPlayer(function (current2) {
                                if (withatt) {
                                    let att2 = get.attitude(player, current2);
            ...
            moveCard: function () {
                ...
                for (let i = 0; i < arguments.length; i++) {
                    if (typeof arguments[i] == 'boolean') {
                        ...
                        if (!arguments[i].some(j => typeof j != 'string')) {
            ...
            useResult: function (result, Evt) {
                ...
                if (result.skill) {
                    let info = get.info(result.skill);
                    if (info.onuse) {
            ...
            useCard: function () {
                ...
                if (typeof this.logAi == 'function' && !next.noai && !get.info(next.card).noai) {
                    let postAi = get.info(next.card).postAi;
                ...
                switch (num) {
                    ...
                    case 6: {
                        ...
                        for (let i = 0; i < lib.inpile.length; i++) {
                            let info = lib.card[lib.inpile[i]];
            ...
            canAddJudge: function (card) {
                ...
                let mod = game.checkMod(card, this, this, 'unchanged', 'targetEnabled', this);
            ...
            logSkill: function (name, targets, nature, logv, audio) {
                ...
                let checkShow = this.checkShow(name);
                ...
                if (info && info.round) {
                    let roundname = name + '_roundcount';
                ...
                if (this._hookTrigger) {
                    for (let i = 0; i < this._hookTrigger.length; i++) {
                        let info = lib.skill[this._hookTrigger[i]].hookTrigger;
                        if (info && info.log) {
            ...
            showTimer: function (time) {
                ...
                let timer = ui.create.div('.timerbar', this);
                this.node.timer = timer;
                ui.create.div(this.node.timer);
                let bar = ui.create.div(this.node.timer);
                ui.refresh(bar);
            ...
            removeAdditionalSkill: function (skill, target) {
                if (this.additionalSkills[skill]) {
                    let additionalSkills = this.additionalSkills[skill];
            ...
            disableSkill: function (skill, skills) {
                if (typeof skills == 'string') {
                    ...
                    let group = lib.skill[skills].group;
            ...
            enableSkill: function (skill) {
                for (let i in this.disabledSkills) {
            ...
            addEquipTrigger: function (card) {
                if (card) {
                    ...
                    if (info.skills) {
                        for (let j = 0; j < info.skills.length; j++) {
                            this.addSkillTrigger(info.skills[j]);
                        }
                    }
                }
                else {
                    let es = this.getCards('e');
                    for (let i = 0; i < es.length; i++) {
            ...
            removeEquipTrigger: function (card, move) {
                if (card) {
                    ...
                    let skills = this.getSkills(null, false);
                    if (info.skills) {
                        for (let j = 0; j < info.skills.length; j++) {
            ...
            addTempSkill: function (skill, expire, checkConflict) {
                ...
                let noremove = this.skills.contains(skill);
                ...
                if (typeof expire == 'string') {
                    ...
                    let roles = ['player', 'source', 'target'];
            ...
            checkConflict: function (skill) {
                if (skill) {
                    ...
                    let forbidlist = lib.config.forbid.concat(lib.config.customforbid);
                    let skills = this.getSkills();
                    for (let i = 0; i < forbidlist.length; i++) {
                        if (lib.config.customforbid.contains(forbidlist[i]) ||
                            ...
                            if (!forbidlist[i].some(j => !skills.contains(j))) {
            ...
            getHistory: function (key, filter, last) {
                ...
                if (!filter) return this.actionHistory[this.actionHistory.length - 1][key];
                    ...
                    let history = this.getHistory(key).slice(0);
            ...
            getLastHistory: function (key, filter, last) {
                let history = this.actionHistory.slice().reverse().find(i => i.isMe());
            ...
            $throwordered1: function (node, nosource) {
                ...
                if (nosource) {
                    ...
                    let parseCalc = function (str) {
                        let per = str.slice(str.indexOf('calc(') + 5, str.indexOf('%'));
                        let add = str.slice(str.indexOf('%') + 1, str.indexOf('px')).replace(/\s/g, '');
            ...
            $throwxy2: function (card, left, top, trans, flipx, flipy) {
                ...
                let node = card.copy('thrown', 'thrownhighlight');
                node.style.left = left;
                node.style.top = top;
                node.hide();
                // node.style.transitionProperty='left,top,opacity,transform';
                ...
                let ay = parseCalc(node.style.top);
                let nx = ax[0] * ui.arena.offsetWidth / 100 + ax[1], ny = ay[0] * ui.arena.offsetHeight / 100 + ay[1];
                let dx = this.getLeft() + this.offsetWidth / 2 - 52 - nx;
                let dy = this.getTop() + this.offsetHeight / 2 - 52 - ny;
                if (flipx) dx = -dx;
                if (flipy) dy = -dy;
                if (trans) {
            ...
            throwDice: function (num) {
                ...
                game.broadcastAll(function (num) {
                    let diceContainer = ui.create.div('.fullsize.dice-container', ui.window);
                    ui.window.classList.add('dicepaused');
                    let dice = ui.create.div('.dice');
                    let side;
