...
module.exports = {
  libFun: (vkCore) => {
    ...
    return {
      ...
      help: {
        'FAQ': `<ul>
        <li>Q：护甲会影响伤害时机相关的技能结算吗？<li>A：会，当伤害因为消耗护甲而被防止时，与其他情况防止伤害的结算方式大致相同，如受伤角色的卖血技能会因为护甲防止了伤害而无法触发<br>
        <li>Q：关于家长麦技能中的“除外”，有更详细的说明吗？<li>A：你不执行奖惩，不能发动技能或使用牌，不能指定目标或被选择为目标（令角色解除除外状态除外）；计算有关全场角色的数据时，不计算你的存在：当你于回合内被除外时，结束你的回合（若当前有卡牌正在结算，则结算后再结束你的回合）。<br>
        <li>Q：若一名角色有出牌阶段限制次数的技能，该角色会因额外的出牌阶段多次发动此技能吗？<li>A：是的，但是一般情况仅限于主动释放的技能（比如下地的『引流』和MEA的『掠财』）。若不做特殊说明，额外出牌阶段结束时，角色回合内的技能使用次数均会清空。<br>
        <li>Q：夜雾和雫lulu的技能改变出牌效果时，影响牌的使用次数吗？<li>A：不影响，牌的使用次数始终在牌使用或打出时计入。特别的，lulu的技能可以改变牌名，有可能影响牌的后续结算；而夜雾的技能不改变牌名，（虽然效果已经变化）与原牌名关联的效果不会受影响（如【初始服】之于【杀】【万箭】【南蛮】）<br>`,
